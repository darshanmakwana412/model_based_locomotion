...
class LocomotionGymEnv(gym.Env):
  ...
  def _build_action_space(self):
    ...
    if motor_mode == robot_config.MotorControlMode.HYBRID:
      ...
      self.action_space = spaces.Box(low = np.array(action_lower_bound, dtype=np.float64),
                                     high = np.array(action_upper_bound, dtype=np.float64))
      ...
      self.action_space = spaces.Box(low = np.array(-torque_limits, dtype=np.float64),
                                     high = np.array(torque_limits, dtype=np.float64))
    else:
      # Position mode
      action_upper_bound = []
      action_lower_bound = []
